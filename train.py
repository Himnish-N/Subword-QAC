...
class CollateFunction:
    def __init__(self, tokenizer, sample, max_seq_len=None):
        self.tokenizer = tokenizer
        self.sample = sample
        self.max_seq_len = max_seq_len

    def collate_fn(self, queries):
        token_id_seqs = [[1] + self.tokenizer(x, **self.sample) + [2] for x in queries]

        length = [len(x) - 1 for x in token_id_seqs]
        if self.max_seq_len is None or self.max_seq_len > max(length) + 1:
            self.max_seq_len = max(length) + 1
...
def get_args():
    ...
    parser.add_argument('--nhid', type=int, default=300) #600 
    ...
    # training
    parser.add_argument('--max_seq_len', type=int, default=20) # 40
    parser.add_argument('--lr', type=float, default=5e-3)
    parser.add_argument('--n_epochs', type=int, default=5) #30
...
# Define a collation function that can be pickled
# def my_collate_fn(batch, tokenizer, sample, max_seq_len):
#     # Replace this with your actual collation logic
#     return collate_fn(batch, tokenizer, sample, max_seq_len)
...
def train(model, optimizer, tokenizer, train_data, valid_data, args):
    ...
    # Prepare partial function for DataLoader
    #collate_function = functools.partial(my_collate_fn, tokenizer=tokenizer, sample=args.sample, max_seq_len=args.max_seq_len)
    collate_function = CollateFunction(tokenizer, args.sample, args.max_seq_len)

    # Using the defined collate function instead of a lambda
    ...
    valid_data_loader = DataLoader(valid_dataset, sampler=SequentialSampler(valid_data),
                                   batch_size=args.bsz, num_workers=args.num_workers,
                                   collate_fn=collate_function.collate_fn)
                                   #collate_fn=collate_function)
