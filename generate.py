...
def get_args():
    ...
    parser.add_argument('--num_workers', type=int, default=12) #16
...
def beam_search(model, hidden, input, best_score, off, beam_size, branching_factor, max_suffix_len):
    ...
    for i in range(max_suffix_len):
        output, hidden = model(input, hidden=hidden)  # Output from the model for the current step
        logp = F.log_softmax(output.squeeze(0), 1)  # Log probabilities of the output tokens

        if i == 0 and off is not None:
            # Masking certain tokens if 'off' tensor is provided
            logp.masked_fill_(off.unsqueeze(1).repeat(1, beam_size, 1).view(bsz * beam_size, -1), -float('inf'))

        score = logp + best_score.view(-1).unsqueeze(1)  # Update scores

        # Handling the end of sequences
        end_score = score[:, 2].view(-1, beam_size)  # Scores for the end of sequence token
        ...
        end_score = end_score[:, :beam_size]
        end_prev_beam_idx = end_prev_beam_idx[:, :beam_size]
        end_scores.append(end_score)
        end_prev_beam_idxs.append(end_prev_beam_idx)
        score[:, 2].fill_(-float('inf'))  # Fill end token scores to prevent them from being picked again

        # Select top tokens from the score
        val, idx0 = score.topk(branching_factor, 1)
        val = val.view(-1, beam_size * branching_factor)
        idx0 = idx0.view(-1, beam_size * branching_factor)
        best_score, idx1 = val.topk(beam_size, 1)

        # Calculate indices for beam and tokens
        prev_beam_idx = idx1 // branching_factor
        new_token_idx = idx0.gather(1, idx1)
        prev_beam_idxs.append(prev_beam_idx)
        new_token_idxs.append(new_token_idx)

        # Prepare inputs for the next step
        input = new_token_idx.view(1, -1)
        hidden_idx = (prev_beam_idx + batch_idx.unsqueeze(1) * beam_size).view(-1)
        hidden = [(h.index_select(0, hidden_idx), c.index_select(0, hidden_idx)) for h, c in hidden]

    # Construct final tokens tensor
    tokens = torch.full((bsz, beam_size, max_suffix_len), 2, dtype=torch.long, device=device)
    pos = (beam_size + torch.arange(beam_size)).unsqueeze(0).repeat(bsz, 1).to(device)

    # Backtracking to find the best sequences
    for i in reversed(range(max_suffix_len)):
        end = pos >= beam_size
        for j in range(bsz):
            if i < len(new_token_idxs):
                tokens[j, ~end[j].bool(), i] = new_token_idxs[i][j, pos[j, ~end[j].bool()]]
            if i < len(prev_beam_idxs):
                pos[j, ~end[j]] = prev_beam_idxs[i][j, pos[j, ~end[j]]]
                pos[j, end[j]] = end_prev_beam_idxs[i][j, pos[j, end[j]] - beam_size]

    # Calculate decode lengths for each sequence
...
#         end = pos >= beam_size
#         for j in range(bsz):
#             tokens[j, 1 - end[j].int(), i] = new_token_idxs[i][j, pos[j, 1 - end[j].int()]]
#             #tokens[j, 1 - end[j], i] = new_token_idxs[i][j, pos[j, 1 - end[j]]]
#             pos[j][1 - end[j]] = prev_beam_idxs[i][j, pos[j, 1 - end[j].int()]]
#             pos[j][end[j]] = end_prev_beam_idxs[i][j, pos[j, end[j]] - beam_size]
#     decode_len = (tokens != 2).sum(2).max(1)[0]
#     return tokens, end_scores[-1], decode_len
...
class CollateFunction:
    ...
    def collate_fn(self, x):
        return gen_collate_fn(x, self.tokenizer, self.args)
...
def generate(model, tokenizer, data, args, seen_set=None, calc_mrl=False):
    ...
    collate_function = CollateFunction(tokenizer, args)
    ...
    data_loader = DataLoader(dataset, sampler=SequentialSampler(dataset), batch_size=bsz, num_workers=args.num_workers,
                            collate_fn=collate_function.collate_fn)
    ...
    for i, batch in enumerate(data_loader):
        ...
        #if True: run this for seeing immediate
